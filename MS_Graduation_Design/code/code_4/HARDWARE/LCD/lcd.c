...
// LCD屏静态显示
void LCD_Static_Display(void)
{
	POINT_COLOR = BLACK; // 画笔颜色：黑色
	LCD_Clear(WHITE);	 // 背景色

	// 状态显示
	LCD_Fill(15, 10, 225, 135, GRAYBLUE);			// 上面的背景色
	LCD_DrawLine(112, 10, 112, 135);				// 画中间的竖线
	LCD_DrawRectangle(15, 10, 225, 135);			// 框框
	LCD_ShowString(30, 15, 210, 20, 24, " DOOR ");	// 显示门口信息
	LCD_DrawLine(15, 42, 225, 42);					// 画线
	LCD_ShowString(30, 45, 210, 40, 24, " GSM  ");	// 显示GSM模块
	LCD_DrawLine(15, 72, 225, 72);					// 画线
	LCD_ShowString(30, 75, 210, 60, 24, " BEEP ");	// 显示蜂鸣器信息
	LCD_DrawLine(15, 102, 225, 102);				// 画线
	LCD_ShowString(30, 105, 210, 60, 24, "RECORD"); // 显示录音信息

	// 错误或成功显示信息
	POINT_COLOR = BLACK;				  // 画笔颜色：黑色
	LCD_Fill(20, 145, 220, 240, YELLOW);  // 中间的背景色
	LCD_DrawRectangle(20, 145, 220, 240); // 框框

	// 显示按键信息
	LCD_Fill(25, 250, 215, 315, LGRAY);								 // 下面的背景色
	LCD_DrawRectangle(25, 250, 215, 315);							 // 框框
	POINT_COLOR = BRED;												 // 画笔颜色：黑色
	LCD_ShowString(35, 255, 215, 220, 16, "KEY0:BEEP/RECORD");		 // 显示蜂鸣器信息
	LCD_ShowString(35, 275, 215, 220, 16, "KEY1:GSM/BMP/REPLAY");	 // 显示蜂鸣器信息
	LCD_ShowString(35, 295, 215, 220, 16, "KEY2:CAMERA/Microphone"); // 显示蜂鸣器信息
}

/**
 * @Description: // LCD屏动态显示
 * @return {*}
 * @Author: 马爽
 * @Date: 2024-01-28 18:49:18
 * 每次刷新时LCD只会覆盖当前字符串大小，如果上次显示的字符太长会覆盖不到！！！！！
 * EG：前一次显示DISABLE，后一次显示enable的话要多加一个' '来覆盖
 */
// LCD屏动态显示
void LCD_Dynamic_Display(void)
{
	POINT_COLOR = RED; // 画笔颜色：红色
	/**
	 * @Description: 显示是否在发短信
	 * @return {*}
	 * @Author: 马爽
	 * @Date: 2024-01-28 18:21:06
	 */
	if (KEY_GSM_flag == 1)
	{
		LCD_ShowString(125, 45, 210, 200, 24, "ENABLE ");
	}
	else
	{
		LCD_ShowString(125, 45, 210, 200, 24, "DISABLE");
	}

	/**
	 * @Description: 显示蜂鸣器是否在响
	 * @return {*}
	 * @Author: 马爽
	 * @Date: 2024-01-28 18:21:10
	 */
	if (KEY_Buzzer_flag == 1)
	{
		LCD_ShowString(125, 75, 210, 200, 24, "ENABLE ");
	}
	else
	{
		LCD_ShowString(125, 75, 210, 200, 24, "DISABLE");
	}
	 ...
	 * @return {*}
	 * @Author: 马爽
	 * @Date: 2024-01-28 18:21:20
	 */
	if (KEY_WS2812B_flag == 1)
	{
		LCD_ShowString(125, 15, 210, 200, 24, "ENABLE ");
	}
	else
	{
		LCD_ShowString(125, 15, 210, 200, 24, "DISABLE");
	}
	 ...
	 * @return {*}
	 * @Author: 马爽
	 * @Date: 2024-02-02 01:09:59
	 */
	if (Microphone_flag == 1)
	{
		LCD_ShowString(125, 105, 210, 200, 24, "ENABLE ");
	}
	else
	{
		LCD_ShowString(125, 105, 210, 200, 24, "DISABLE");
	}
}
